...
function initTela() {
    recebimentosTable = document.querySelector('#recebimentos-table');
    actionButton = document.querySelector('#action-btn');
    descartarButton = document.querySelector('#descartar-form-btn');
    form = document.querySelector('#recebimentos-form');
    formShield = document.querySelector('#recebimentos-crud-shield');
    formTitle = document.querySelector('#titulo-form');
    inputId = document.querySelector('#recebimentos-id');
...
function atualizarTela() {
    // Limpando tabela
    while(recebimentosTable.rows.length > 1) {
        recebimentosTable.deleteRow(1);
    }

    movimentacaoCaixaDao.carregarMovimentacoes(filtro.value, 'C', paginaAtual-1, tamanhoPagina, (result, err) => {

        if (err) {
            let msgErro = `Ocorreu um erro ao carregar os recebimentos: ${err}`;
        ...
        if (result) {
            result.registros.forEach(registro => {
                let row = recebimentosTable.insertRow();
